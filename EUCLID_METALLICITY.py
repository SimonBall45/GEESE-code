...
# ===========================================================
EUCLID['68metalclean'] = EUCLID['phz_pp_68_stellarmetallicity'].str.replace(r'[\[\]]', '', regex=True)
EUCLID[['68metallow','68metalhigh']] = EUCLID['68metalclean'].str.split(',', n=1, expand=True)
EUCLID.replace(r'^\s*$', np.nan, regex=True)
EUCLID.dropna(subset=['68metallow','68metalhigh'],inplace=True)
EUCLID['68metallow'] = pd.to_numeric(EUCLID['68metallow'], errors='coerce')
EUCLID['68metalhigh'] = pd.to_numeric(EUCLID['68metalhigh'], errors='coerce')
EUCLID.dropna(subset=['68metallow','68metalhigh'],inplace=True)
...
metal = EUCLID['phz_pp_median_stellarmetallicity']
metal68low = EUCLID['68metallow']
metal68high = EUCLID['68metalhigh']
EUCLID['metal_err_low'] = np.absolute(metal - metal68low)
EUCLID['metal_err_high'] = np.absolute(metal68high - metal)
...
metal_err = np.array([EUCLID['metal_err_low'], EUCLID['metal_err_high']])

# =============================================
# Weighted mean mass
# =============================================

metal_below = metal < 1.45
metal_above = metal > 1.45

#print(np.where(metal >1.5))
...
rms_below = rms(hrmod[metal_below], 213)
rms_above = rms(hrmod[metal_above], 74)


# ---- total variance = measurement + intrinsic scatter ----
sigma2_below = hrerr[metal_below]**2 + rms_below**2
sigma2_above = hrerr[metal_above]**2 + rms_above**2
...
print("Weighted mean HR (Metallicity < 1.45):", below_mean)
print("Weighted mean HR (Metallicity > 1.45):", above_mean)


# =============================================
# Chi squared calc
# =============================================
step = np.where(metal < 1.45, below_mean, above_mean)
def chi(x, w, N):
...
p0 = [-0.005, mass_step, 1.45, 1]

values, covs = curve_fit(sigmoid, metal, hrmod, p0,)
sigmoidData = sigmoid(metal, *values)
...
plt.xlabel(r'Metallicity', fontsize='30', color='grey')
