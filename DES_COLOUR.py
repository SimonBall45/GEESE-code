...
colorerr= meta['HOST_COLOR_ERR']

# =============================================
# Weighted mean SFR
# =============================================

color_below = np.where(color < 1.1)
color_above = np.where(color > 1.1)
...
rms_below = rms(hrmod[color_below], 720)
rms_above = rms(hrmod[color_above], 536)


# ---- total variance = measurement + intrinsic scatter ----
sigma2_below = MUcalcerr[color_below]**2 + rms_below**2
sigma2_above = MUcalcerr[color_above]**2 + rms_above**2
...
print("Weighted mean HR (color < 0.9):", below_mean)
print("Weighted mean HR (color > 0.9):", above_mean)


# =============================================
# Chi squared calc
# =============================================
step = np.where(color<1.1, below_mean, above_mean)
def chi(x, w, N):
    return ((np.sum(w*(x-hrmod)**2))/(N-1))

rmschi = rms(hrmod, 1256)
sigma2chi = MUcalcerr**2 + rmschi**2
weightschi = 1.0 / sigma2chi

chisq_step = chi(step, weightschi, 1256)
...
p0 = [-0.005, mass_step, 1.1, 1]

values, covs = curve_fit(sigmoid, color, hrmod, p0,)
sigmoidData = sigmoid(color, *values)
...
chisq_sigmoid=chi(sigmoidData, weightschi, 1256)
...
# Scatter with errors
# note error fro colour set to 0 due to data in DES reading all as -999
plt.errorbar(
    x=color,
    y=hrmod,
    yerr=MUcalcerr,
...
plt.xlabel(r'Colour', fontsize = '30', color='grey')
...
ax.tick_params(axis='x', which='minor', length=3, labelsize='20')
