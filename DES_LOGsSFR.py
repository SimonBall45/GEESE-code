...
sSFR_below = np.where(logsSFR < -9.6)
sSFR_above = np.where(logsSFR > -9.6)
...
rms_below = rms(hrmod[sSFR_below], 334)
rms_above = rms(hrmod[sSFR_above], 370)


# ---- total variance = measurement + intrinsic scatter ----
sigma2_below = MUcalcerr[sSFR_below]**2 + rms_below**2
sigma2_above = MUcalcerr[sSFR_above]**2 + rms_above**2
...
print("Weighted mean HR (Log(sSFR) < -9.6):", below_mean)
print("Weighted mean HR (Log(sSFR) > -9.6):", above_mean)


# =============================================
# Chi squared calc
# =============================================
step = np.where(logsSFR <-9.6, below_mean, above_mean)
def chi(x, w, N):
    return ((np.sum(w*(x-hrmod)**2))/(N-1))

rmschi = rms(hrmod, 704)
sigma2chi = MUcalcerr**2 + rmschi**2
weightschi = 1.0 / sigma2chi

chisq_step = chi(step, weightschi, 704)
...
p0 = [-0.005, mass_step, -9.6, 1]

values, covs = curve_fit(sigmoid, logsSFR, hrmod, p0,)
sigmoidData = sigmoid(logsSFR, *values)
print(covs)
print(np.diag(covs))
print(np.sqrt(np.diag(covs)))
m = np.linspace(-11, -8,1000)
plt.plot(m, sigmoid(m,*values))

print(values)

chisq_sigmoid=chi(sigmoidData, weightschi, 704)
...
plt.xlabel(r'$\log(\mathrm{sSFR})$', fontsize='30', color='grey')
